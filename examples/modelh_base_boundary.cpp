...
int main(int argc, char **argv)
{
    ...
    // std::vector<std::string> names = {"phi","iqxphi","iqyphi","sigxx","sigxy","vx","vy","w","P"};
    // std::vector<BoundaryDirection> dir = {BoundaryDirection::xminus,BoundaryDirection::xplus,BoundaryDirection::yminus,BoundaryDirection::yplus};
    // for (int i = 0; i<names.size(); i++){
    //     for (int j = 0; j<dir.size(); j++){
    //         system.addBoundaryCondition(names[i],BoundaryConditions(BoundaryType::Dirichlet,dir[j],0));
    //     }
    // }
    system.addBoundaryCondition("phi",BoundaryConditions(BoundaryType::VonNeumann,BoundaryDirection::xminus,0));
    system.addBoundaryCondition("phi",BoundaryConditions(BoundaryType::VonNeumann,BoundaryDirection::xplus,0));
    system.addBoundaryCondition("phi",BoundaryConditions(BoundaryType::VonNeumann,BoundaryDirection::yminus,0));  
    system.addBoundaryCondition("phi",BoundaryConditions(BoundaryType::VonNeumann,BoundaryDirection::yplus,0));
